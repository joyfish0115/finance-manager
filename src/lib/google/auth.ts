/**
 * Google OAuth 2.0 with PKCE — 適合純前端 SPA，不需要 Client Secret。
 *
 * 流程：
 *   1. signIn(): 產生 code_verifier，導向 Google 授權頁
 *   2. Google 把使用者導回 redirect_uri，URL 帶 ?code=xxx
 *   3. handleAuthCallback(): 用 code + verifier 換 access_token / refresh_token
 *   4. refreshAccessToken(): access_token 過期前用 refresh_token 換新的
 */

import { GOOGLE_CONFIG, OAUTH_ENDPOINTS, getRedirectUri } from './config'
...
function base64UrlEncode(bytes: Uint8Array): string {
  // ArrayBuffer → base64 → URL-safe（去掉 padding，+ → -，/ → _）
...
export async function signIn(): Promise<void> {
  ...
  // verifier / state 存 sessionStorage（重整就清掉，避免重放）
